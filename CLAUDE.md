...
- **Phase 3b2** — TERMINEE. Bulk import via 2 voies : script Python CLI (`scripts/add_cards.py`, mode 1 = OCR + CSV via Gemini Batch API, mode 2 = commit avec Y/N/A prompt sur fallback) et web batch tab (`/submit` 4e onglet "Batch", ≤15 photos sync, analyse parallèle → review queue → commit séquentiel). **Status fallback auto** côté serveur (`POST /api/cards` : conflict for_sale → retry status='collection' + flag `fallback: 'for_sale_to_collection'` + `reason`) bénéficie aux 3 entry points (web scanner, web batch, Python script). Helper `lib/utils/resize-image.ts` default lowered 1600→1024px (~15× économie tokens Gemini), wired aussi dans LotForm + BatchForm. Web scanner affiche un toast info quand fallback. Python : 5 modules helpers (`cache.py` SHA256+JSON, `image_utils.py` Pillow resize+pre-filter, `csv_io.py`, `gemini_batch.py` Batch API wrapper, `iris_client.py` Supabase login + /api/enrich + /api/cards). 15 tests pytest + 2 tests vitest (242 vitest total). Économie : web scanner 15× moins cher, script 30× (200 cartes : $0.12 → $0.004). Pas de migration DB.
