...
- **Phase 3a** — TERMINEE. Cron pricing quotidien Vercel `0 2 * * *` UTC via TCGdex (`POST /api/prices/update`, protégé `CRON_SECRET`). Bulk mode (LIMIT 200, parallélisme 10, oldest first via `cm_updated_at ASC NULLS FIRST`) + single-card mode (`?card_id=X` derrière auth Supabase). 3 helpers purs : `categorize-pricing-card` (skip variant/KO/ZH, backfill auto pour cartes sans `card_id_tcg`), `format-staleness` (4 tons : fresh/stale/old/never), `recalc-suggested-price` (heuristique qui préserve les édits manuels). 2 composants UI : `<PriceFreshnessBadge>` + `<RefreshPriceButton>` intégrés dans VintedRow / StockRow / PokedexDrawer. `vercel.json` cron schedule ajouté. **223 tests** (+24 vs Phase 2.1), 0 lint warning, 0 type error. Aucune nouvelle migration (toutes colonnes existaient depuis Phase 1).
