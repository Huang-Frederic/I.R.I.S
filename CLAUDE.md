...
- **Phase 3b2** — TERMINEE (révisée). Bulk import 100% web via la 4e tab `/submit` "Batch" (≤30 photos sync, analyse parallèle → review queue carte-par-carte avec Préc/Suiv → commit séquentiel). Status fallback auto côté serveur (`POST /api/cards` : conflict for_sale → retry status='collection' + flag `fallback`) bénéficie aux 2 entry points web (scanner unitaire + batch tab) avec `<DuplicateForSaleModal>` qui s'affiche post-save sur le scanner. Helper `lib/utils/resize-image.ts` default lowered 1600→1024px (~15× économie tokens Gemini), wired dans CardScanForm + LotForm + BatchForm. Status passe de 3 boutons radio à un dropdown + champ Quantité à droite (Mobile + Batch identiques). Batch fetch les Pokédex registered au start pour afficher un warning ⚠️ par carte. Bouton "Tout enregistrer" disabled tant que toutes les cartes pas visitées. Endpoint `GET /api/pokedex/registered` ajouté. **Le script Python a été dropé** (initialement Tasks 4-11 du plan 3b2) — UX web suffit pour ≤30 cartes, plus de duplication TS↔Python à maintenir, économie d'env Python en moins. Cap raisonnable : 30 photos par batch (Vercel 60s + Gemini Tier 1 15 req/min). 242 tests vitest, 0 lint warning, 0 type error.
