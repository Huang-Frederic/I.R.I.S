...
- **Phase 4** — A FAIRE. Bulk vendu (selecteur multi-cartes vendues ensemble + prix divisé) + Refining Gemini tokens (audit + optimisation pipeline coût).
- **Phase 5** — A FAIRE. Passage à 2 users (RLS multi-tenant) + Import one-shot du profil Vinted existant.
- **Phase 6** — A FAIRE. Dashboard (KPIs incluant tokens consommés et coût/jour) + polish PWA (install prompt, icônes, manifest).
