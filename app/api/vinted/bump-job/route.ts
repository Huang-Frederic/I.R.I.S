...
export async function POST(request: Request) {
  ...
  if (isCard) {
    const { data: card, error: cardError } = await supabase
      .from('cards')
      .select('id, status, card_name, card_id_tcg, set_code, set_name, language, condition, variant, pokemon_name, suggested_price')
      .eq('id', card_id!)
      .single();
    ...
    if (!myListing?.vinted_listing_id) {
      return apiError('no_listing', { status: 409, message: 'Aucune annonce Vinted existante pour cette carte' });
    }
    ...
    if (activeJob) {
      return apiError('job_already_queued', { status: 409, message: 'Un job de bump est déjà en cours pour cette carte' });
    }
  ...
  const { data: lot, error: lotError } = await supabase
    .from('lots')
    .select('id, status, name, price, language, condition')
    .eq('id', lot_id!)
    .single();
  ...
  if (!myLotListing?.vinted_listing_id) {
    return apiError('no_listing', { status: 409, message: 'Aucune annonce Vinted existante pour ce lot' });
  }
  ...
  if (activeLotJob) {
    return apiError('job_already_queued', { status: 409, message: 'Un job de bump est déjà en cours pour ce lot' });
  }
