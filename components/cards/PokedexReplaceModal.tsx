...
function formatVariant(v: string | null, standardLabel: string): string {
  if (!v) return standardLabel;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-surface border-border w-full max-w-2xl rounded-lg border p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <p className="text-text-muted mt-1 text-sm">
              {t('pokedexReplaceSubtitle', { name: displayPokemonName(existingCard) })}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="my-6 flex items-center justify-center gap-4">
          <CardPanel
            label={t('pokedexReplaceExisting')}
            thumbUrl={existingThumb}
            title={displayCardName(existingCard)}
            subtitle={`${existingCard.rarity} · ${existingCard.language} · ${formatVariant(existingCard.variant, standardLabel)}`}
            onZoomClick={existingThumb ? () => setZoomSrc(existingThumb) : undefined}
            zoomLabel={t('pokedexReplaceZoomAria')}
            noImageLabel={tCommon('noImage')}
          />
          <ArrowRight className="text-text-muted h-6 w-6 shrink-0" />
          <CardPanel
            label={t('pokedexReplaceNew')}
            thumbUrl={newThumb}
            title={displayCardName(newCardSummary)}
            subtitle={`${newCardSummary.rarity} · ${newCardSummary.language} · ${formatVariant(newCardSummary.variant, standardLabel)}`}
            highlight
            onZoomClick={newThumb ? () => setZoomSrc(newThumb) : undefined}
            zoomLabel={t('pokedexReplaceZoomAria')}
            noImageLabel={tCommon('noImage')}
          />
        </div>
        ...
        <div className="border-border bg-surface-2 mb-4 rounded border p-3 text-xs">
          <p className="text-text-muted">{t('pokedexReplaceNote')}</p>
        </div>

        {hasForSaleConflict && (
          <p className="text-text-muted mb-3 text-xs">
            {t('pokedexReplaceConflictHint')}
          </p>
        )}
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
          ...
          >
            {submitting ? t('pokedexReplaceWaiting') : t('pokedexReplaceToStock')}
          </button>
          <button
            type="button"
            onClick={() => onConfirm('for_sale')}
            disabled={submitting || hasForSaleConflict}
            title={hasForSaleConflict ? t('pokedexReplaceConflictTitle') : undefined}
        ...
        {zoomSrc && (
          <CardZoomModal src={zoomSrc} alt={t('pokedexReplaceZoomAlt')} onClose={() => setZoomSrc(null)} />
        )}
      </div>
    </div>
  );
