import { ScanLine, BookOpen, Package, Tag, Settings, BarChart3, TrendingUp, Activity, type LucideIcon } from 'lucide-react';

export interface NavItem {
  href: string;
  /** Translation key under the `nav` namespace (e.g. 'dashboard'). */
  labelKey: 'dashboard' | 'scanner' | 'pokedex' | 'stock' | 'vinted' | 'prices' | 'options' | 'logs';
  icon: LucideIcon;
}
