...
import { ChevronRight, ChevronLeft, ChevronUp, Bot, type LucideIcon } from 'lucide-react';
...
/** Static lead-in slot rendered at the bottom-right start of the queue's
 *  snake path — the very next card (connected by a chevron) is unambiguously
 *  "next up", with the whole queue winding upward from there. Never
 *  draggable/sortable — it's not a real item. */
export function PosterCardStartSlot() {
  return (
    <div
      ...
      title="Le bot commence ici"
      aria-hidden
    >
      <Bot className="text-text-faint h-8 w-8" />
    </div>
  );
}
