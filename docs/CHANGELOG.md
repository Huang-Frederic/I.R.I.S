...
- **Fondations** — tables `vinted_queue` (file manuelle « à poster »), `vinted_bot_schedule` (créneaux jour/heure), `vinted_bot_config` (quota + seuil de repost), `vinted_sessions` (cookies collés depuis l'UI, synchronisés côté agent), `vinted_agent_logs` (miroir de la console de l'agent, sans accès serveur). Boucle `_scheduling_loop` côté Python (`vinted-agent/main.py`) + logique de décision pure et testée (`scheduler.py`'s `decide_next_action`) : poste le premier de la file si elle n'est pas vide, sinon reposte l'annonce périmée la plus ancienne — jamais l'inverse, règle non configurable depuis l'UI.
- **UI de monitoring** — `StatusBar` (statut agent, quota du jour, prochain post estimé, switch de compte), `AlertBanner`, pipeline de file en scroll horizontal, pool de reposts, formulaires quota/planning/cookies, flux de logs — tout empilé verticalement sur une page unique au départ.
- **Split de page** — le monitoring quitte `/vinted` pour sa propre route `/vinted/bot` avec entrée de nav dédiée ; boutons « Mettre en premier »/« Poster maintenant » par carte ; correctif de cohérence : `vinted_queue` est désormais nettoyé dès qu'un post/bump manuel est créé, plus seulement par la boucle du bot.
- **Refonte « serpent » + groupes** — la page jugée peu claire (« c'est pas clair ») : le pipeline horizontal scrollable devient une grille en flex-wrap qui se lit avec le scroll normal de la page, groupée visuellement (cadres pointillés/teintés) par catégorie — « Pokémon FR », « Pokémon JP », « Riftbound », « Magic », etc., dérivée de la langue d'une carte ou de la marque/langue d'un lot (`lib/vinted/group-key.ts`, `lib/vinted/group-sort.ts`, TDD). Ordre de priorité des groupes configurable et persisté (`vinted_bot_config.group_priority`, éditeur glisser-déposer). Tous les réglages (quota, planning, cookies, ordre des groupes) migrent derrière un bouton « Paramètres » ouvrant une modale, ne laissant sur la page que la file, les reposts et les logs.
- **Pool de reposts groupé + ordre manuel réel** — le pool de reposts (jusque-là un affichage lecture seule trié par ancienneté) reçoit le même système de groupes que la file (`GroupedRepostGrid`, badges « Repost · {groupe} »), avec un glisser-déposer qui a un effet réel sur le bot : nouvelle colonne `repost_position` (nullable) sur `card_listings`/`lot_listings`, lue par l'agent Python (`sort_repost_candidates`, TDD pytest) qui priorise une position manuelle avant de retomber sur l'ancienneté. Bouton « Reposter maintenant » pour un repost immédiat (`POST /api/vinted/post-job` avec `job_type: 'repost'`, nouveau). Bug découvert et corrigé au passage : `useMonitoringData.ts` n'interrogeait jamais `lot_listings` pour le pool de reposts — les lots (Riftbound, Magic…) n'y apparaissaient donc jamais, alors que le bot les repostait déjà.
- Tech debt connue (voir [TECH_DEBT.md](TECH_DEBT.md)) : la boucle Python récupère désormais tous les candidats au repost éligibles au lieu d'un `LIMIT 1` par table (nécessaire pour que l'ordre manuel soit toujours pris en compte) ; `GroupedRepostGrid` n'affiche aucun message quand le pool est vide, contrairement à `GroupedQueueGrid`.
- 241 fichiers de tests / 2113 tests verts côté I.R.I.S, suite `pytest` verte côté `vinted-agent/` (hors un échec préexistant et sans rapport à cette phase, `vinted_api_test.py::test_build_listing_payload`).
