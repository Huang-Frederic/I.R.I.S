...
describe('parsePlayinEvents', () => {
  ...
  it('extracts every timed event under its date header', () => {
    expect(events).toHaveLength(4);
    expect(events[0]).toMatchObject({ dateHeader: 'Mercredi 22 Juillet', hh: 14, mm: 30, name: 'Pokémon Coloriage', priceText: 'Gratuit' });
    expect(events[2]).toMatchObject({ dateHeader: 'Mercredi 22 Juillet', hh: 15, mm: 0, name: 'Pokémon Initiation', priceText: '5,00 €' });
  });
